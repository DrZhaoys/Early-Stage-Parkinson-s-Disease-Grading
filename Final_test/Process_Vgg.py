...
# 加载预训练的VGG16模型并修改最后一层
model = models.vgg16(pretrained=True)  # 加载预训练模型
model.classifier[6] = nn.Linear(model.classifier[6].in_features, 3)  # 修改最后一层以适应三分类任务
...
for i in range(3):  # 三分类任务
    ...
    plt.plot(fpr[i], tpr[i], label=f'VGG16 class {i} (AUC = {roc_auc[i]:.2f})')
...
# 输出模型的准确率
print(f'VGG16 Accuracy: {valid_accuracy * 100:.2f}%')
