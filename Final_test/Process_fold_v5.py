...
if __name__ == "__main__":
    ...
    if mode == "train":
        ...
        for fold, (train_idx, valid_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels), 1):
            ...
            for epoch in range(50):  # 限制到20个epoch
            ...
            # 保存模型
            model_save_path = f'./saved_models/resnet101_{fold}_final.pth'
        ...
        model = models.resnet101(pretrained=True)  # 加载模型结构
        model.fc = nn.Linear(model.fc.in_features, 4)
        model = model.to(device)
        loss_fn = nn.CrossEntropyLoss()
        # 遍历保存的模型并测试每个模型
        ...
        for fold in range(1, 6):  # 假设你有5折模型
            model_path = os.path.join(saved_models_dir, f'resnet101_{fold}_final.pth')
