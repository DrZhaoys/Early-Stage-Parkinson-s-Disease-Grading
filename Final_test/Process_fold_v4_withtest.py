...
if __name__ == "__main__":
    ...
    dataset = VideoFrameDataset(file_path='./data/train3.txt', transform=data_transforms)
    ...
    if mode == "train":
        ...
        for fold, (train_idx, valid_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels), 1):
            ...
            for epoch in range(10):  # 限制到20个epoch
            ...
            # 保存模型
            model_save_path = f'./saved_models/fold_{fold}_model.pth'
        ...
        test_dataset = VideoFrameDataset(file_path='./data/test.txt', transform=data_transforms)
        test_dataloader = DataLoader(test_dataset, batch_size=64, shuffle=False)

        model = models.resnet101(pretrained=False)  # 加载模型结构
        model.fc = nn.Linear(model.fc.in_features, 3)
        model = model.to(device)
        loss_fn = nn.CrossEntropyLoss()
        # 遍历保存的模型并测试每个模型
        ...
        for fold in range(1, 6):  # 假设你有5折模型
            model_path = os.path.join(saved_models_dir, f'fold_{fold}_model.pth')
            ...
            test_metrics = calculate_metrics(test_labels, test_preds, num_classes=3)
