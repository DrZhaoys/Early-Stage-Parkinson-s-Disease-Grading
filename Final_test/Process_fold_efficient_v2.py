...
if __name__ == "__main__":
    mode = "train"  # 训练模式："train"，测试模式："test"
    ...
    if mode == "train":
        ...
        for fold, (train_idx, valid_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels), 1):
            ...
            # 设置 matplotlib 为交互模式以实时更新 loss 曲线
            plt.ion()  # 开启交互模式
            fig, ax = plt.subplots(figsize=(8, 6))  # 创建一个图形窗口
            ax.set_xlabel('Epoch')
            ax.set_ylabel('Loss')
            ...
            for epoch in range(50):  # 限制到50个epoch
                ...
                # 动态更新 loss 曲线
                ax.clear()  # 清除之前的绘图
                ax.plot(range(1, epoch + 2), train_losses, label='Train Loss', color='blue')
                ax.plot(range(1, epoch + 2), valid_losses, label='Valid Loss', color='orange')
                ax.legend()  # 添加图例
                ax.set_xlabel('Epoch')
                ax.set_ylabel('Loss')
                ax.set_title(f'Loss Curve (Fold {fold})')
                plt.draw()  # 绘制更新后的图像
                plt.pause(0.1)  # 暂停一小段时间以便显示更新

            # 关闭交互模式并显示最终图像
            plt.ioff()  # 关闭交互模式
            plt.show()  # 显示最终的 loss 曲线窗口

            # 绘制训练曲线（保留你原来的函数调用，如果你仍想在训练结束后看到静态图像）
            ...
            # 保存模型
            model_save_path = f'./saved_models/efficientnet_b0_{fold}_final.pth'
        ...
        precision_scores = [np.mean([m['metrics'][f'Class {i}']['precision'] for i in range(3)]) for m in
                            all_fold_metrics]
        ...
        for fold in range(1, 6):  # 假设你有5折模型
            model_path = os.path.join(saved_models_dir, f'efficientnet_b0_{fold}_final.pth')
