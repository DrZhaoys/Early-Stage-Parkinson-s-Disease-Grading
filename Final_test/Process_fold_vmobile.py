...
def test(dataloader, model, threshold=0.1):
    ...
    with torch.no_grad():
        for X, y in tqdm(dataloader, desc="Testing"):
            ...
            pred_classes = (pred_probs[:, 1] > threshold).long()
...
def calculate_metrics(labels, preds, num_classes):
    ...
    class_report = classification_report(labels, pred_classes,
...
if __name__ == "__main__":
    mode = "test"  # 可更改为 "test"
    model_path = "./saved_models/fold_1_mobile.pth"
    new_data_path = "./new_data/test_data.txt"
    ...
    if mode == "train":
        ...
        for fold, (train_idx, valid_idx) in enumerate(kfold.split(np.zeros(len(labels)), labels), 1):
            ...
            # 关键修改：调整分类器最后一层
            model.classifier[1] = nn.Linear(model.last_channel, 4)  # 正确修改MobileNet分类层
            ...
            model_save_path = f'./saved_models/fold_{fold}_mobile.pth'
        ...
        precision_scores = [np.mean([m['metrics'][f'Class {i}']['precision'] for i in range(4)]) for m in
                            all_fold_metrics]
        recall_scores = [np.mean([m['metrics'][f'Class {i}']['recall'] for i in range(4)]) for m in all_fold_metrics]
        f1_scores = [np.mean([m['metrics'][f'Class {i}']['f1-score'] for i in range(4)]) for m in all_fold_metrics]
        ...
        # 初始化模型时必须先修改分类层结构
        model = models.mobilenet_v2(pretrained=False)
        model.classifier[1] = nn.Linear(model.last_channel, 4)  # 关键修改
        model = model.to(device)
        loss_fn = nn.CrossEntropyLoss()
        ...
        for fold in range(1, 6):
            model_path = os.path.join(saved_models_dir, f'fold_{fold}_mobile.pth')
            ...
            print(f"  Average AUC: {np.mean(list(test_auc.values())):.4f}")
