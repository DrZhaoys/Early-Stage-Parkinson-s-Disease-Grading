...
wdsr_output_folder = 'E:/USP/WDSR_2x'  # WDSR超分结果输出路径
save_csv_path = 'super_res_results_less.csv'  # 保存PSNR和SSIM结果
...
if TRAIN:
    ...
    # 加载新的测试数据集并裁剪后再插值
    def preprocess_test_image(new_test_folder, scale=4):
        img = Image.open(new_test_folder)
        img = transforms.CenterCrop((444, 580))(img)  # 裁剪到 scale 的整数倍
        img = transforms.ToTensor()(img)

        # 下采样和双三次插值恢复
        low_res = nn.functional.interpolate(img.unsqueeze(0), scale_factor=1 / scale, mode='bicubic',
                                            align_corners=False)
    ...
    with torch.no_grad():
        for filename in tqdm(test_images):
            ...
            # 通过WDSR模型进行超分辨率重建
            output = model(bicubic_img.unsqueeze(0))
            output = output.cpu().squeeze(0).permute(1, 2, 0).numpy()

            # 保存WDSR输出结果
            save_path = os.path.join(wdsr_output_folder, filename)
            cv2.imwrite(save_path, (output * 255).astype('uint8'))

    # =============== PSNR 和 SSIM 评估 ====================
    for filename in os.listdir(wdsr_output_folder):
        sr_img = cv2.imread(os.path.join(wdsr_output_folder, filename))
        hr_img = cv2.imread(os.path.join(new_test_folder, filename))  # 使用裁剪后的高分辨率图像作为参考
