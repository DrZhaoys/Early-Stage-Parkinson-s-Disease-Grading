...
wdsr_output_folder_2x = 'E:/USP/WDSR_2x'
...
#
#     # 保存插值结果
#     save_path = os.path.join(bicubic_output_folder_8x, filename)
...
# 数据加载器
train_dataset = BicubicDataset(bicubic_output_folder_2x, transform=transform)
...
if TRAIN:
    ...
    with torch.no_grad():
        for filename in tqdm(test_images):
            ...
            # 保存WDSR输出结果
            save_path = os.path.join(wdsr_output_folder_2x4x, filename)
