...
class WDSR_SRResNet(nn.Module):
    def __init__(self, num_blocks=16, in_channels=3, scale=2, block_type='A'):
...
class PreprocessDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        low_res = torch.nn.functional.interpolate(img.unsqueeze(0), scale_factor=0.5, mode='bicubic').squeeze(0)
        return low_res, img
...
model = WDSR_SRResNet(block_type=block_type, scale=2).to(device)
...
if TRAIN:
    ...
    # 保存模型
    torch.save(model.state_dict(), 'wdsr_srresnet_model_real_A_2x.pth')
    print("Model saved successfully!")
else:
    # 加载已保存的模型
    model.load_state_dict(torch.load('wdsr_srresnet_model_real_A_2x.pth'))
    model.eval()  # 设置为评估模式
    ...
    with torch.no_grad():
        ...
        df.to_csv("psnr_ssim_results_new_data_A_2x.csv", index=False)
