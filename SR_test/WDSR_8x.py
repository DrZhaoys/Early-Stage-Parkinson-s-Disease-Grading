...
class WDSR_SRResNet(nn.Module):
    def __init__(self, num_blocks=16, in_channels=3, scale=8, block_type='A'):
...
class PreprocessDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        low_res = torch.nn.functional.interpolate(img.unsqueeze(0), scale_factor=0.125, mode='bicubic').squeeze(0)
        return low_res, img
...
model = WDSR_SRResNet(block_type=block_type, scale=8).to(device)
...
if TRAIN:
    ...
    # 保存模型
    torch.save(model.state_dict(), 'wdsr_srresnet_model_real_A_8x.pth')
    print("Model saved successfully!")
else:
    # 加载已保存的模型
    model.load_state_dict(torch.load('wdsr_srresnet_model_real_A_8x.pth'))
    model.eval()  # 设置为评估模式
    print("Model loaded successfully for evaluation!")

    # 加载新的数据集并进行 PSNR 和 SSIM 评估
    new_data_dir = 'E:/USP/test/frame_crop/'  # 新数据集路径
    ...
    with torch.no_grad():
        for idx, (low_res, high_res) in enumerate(new_loader):
            low_res = low_res.to(device)  # 将 low_res 移动到 GPU 上
            high_res = high_res.to(device)

            # 模型输出超分辨率结果
            outputs = model(low_res)

            # 转换为 NumPy 数组进行 PSNR 和 SSIM 计算
        ...
        df.to_csv("psnr_ssim_results_new_data_A_8x.csv", index=False)

        print("PSNR and SSIM for each image have been saved to 'psnr_ssim_results_new_data_A_1.csv'")
